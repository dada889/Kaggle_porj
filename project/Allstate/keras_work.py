...
train = pd.read_csv(data_path('Allstate_Claims_Severity') + 'train.csv')
test = pd.read_csv(data_path('Allstate_Claims_Severity') + 'test.csv')
...
for inTr, inTe in folds.split(y):
    # print inTr.shape, inTe.shape
    xtr = xtrain[inTr]
    ytr = y[inTr]
    xte = xtrain[inTe]
    yte = y[inTe]
